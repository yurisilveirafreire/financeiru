...
let demoExpenses = [
  ...
  { id:"e5", userId:"u1", userName:"Você",    description:"Netflix/Spot", amount:75,   category:"lazer",       date:"18/05/2025", month:"2025-05", createdAt:8 },
...
export default function App() {
  ...
  if (screen==="login") return (
    <LoginScreen loading={loading} toast={toast}
      ...
      onDemo={()=>{setDemo(true);setUser({uid:"u1",email:"voce@casa.com",displayName:"Você"});setScreen("app");}}
    />
  );
  ...
  return(
    <div style={S.loginRoot}>
      ...
      <div style={S.loginCard}>
        ...
        <h1 style={S.loginTitle}>financeirU</h1>
        <p style={S.loginSub}>Finanças da casa, juntos</p>
        ...
        <div style={S.divider}><span style={{background:"#1e293b",padding:"0 12px",color:"#475569"}}>ou</span></div>
...
function Dashboard({incomes,expenses,categories,totalIn,totalOut,balance,goal,month,onGoal}){
  ...
  const byPerson={};
  [...incomes,...expenses].forEach(x=>{ if(!byPerson[x.userName]) byPerson[x.userName]={in:0,out:0}; });
  incomes.forEach(i=>{ byPerson[i.userName].in+=Number(i.amount); });
  expenses.forEach(e=>{ byPerson[e.userName].out+=Number(e.amount); });
    ...
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
        {[
          ...
          {label:"Saldo",   val:fmt(balance), icon:balance>=0?"✅":"⚠️",color:balance>=0?"#6366f1":"#f59e0b",sub:balance>=0?"No positivo!":"Atenção!"},
        ].map(c=>(
      ...
      <div style={S.card}>
        ...
        {Object.keys(byCat).length===0 && <p style={S.empty}>Nenhum gasto registrado</p>}
...
// ================================================================
// SHARED: FORM MODAL (entrada ou gasto)
// ================================================================
      ...
      <div style={S.modal}>
        ...
        <div style={S.fg}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
            ...
            <button style={{...S.btnSm,fontSize:11}} onClick={()=>setNewCat(!newCat)}>{newCat?"Cancelar":"+ Nova categoria"}</button>
          </div>
          {newCat?(
            <div style={{background:"#0f172a",borderRadius:10,padding:12,marginBottom:8}}>
              <div style={{display:"flex",gap:8,marginBottom:8}}>
                <input style={{...S.input,flex:"0 0 60px",textAlign:"center",fontSize:20,padding:"6px"}} value={ncIcon} onChange={e=>setNcIcon(e.target.value)} placeholder="🏷️"/>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:6}}>
              {validCats.map(c=>(
                <button key={c.id} onClick={()=>setCat(c.id)}
                  style={{background:cat===c.id?c.color+"33":"#0f172a",border:`2px solid ${cat===c.id?c.color:"#334155"}`,borderRadius:8,padding:"8px 10px",cursor:"pointer",display:"flex",alignItems:"center",gap:8,color:"#e2e8f0",fontSize:13,fontWeight:600,transition:"all .15s"}}>
...
function Incomes({incomes,categories,onAdd,onEdit,onDelete,onAddCat}){
  ...
  return(
    <div>
      ...
      {form && (
        <ItemForm type="income" categories={categories}
...
function Expenses({expenses,categories,onAdd,onEdit,onDelete,onAddCat}){
  ...
  return(
    <div>
      ...
      {form && (
        <ItemForm type="expense" categories={categories}
          item={form==="new"?null:form}
          onAddCat={onAddCat}
          onSave={d=>{ form==="new"?onAdd(d):onEdit(form.id,d); setForm(null); }}
          onClose={()=>setForm(null)}/>
...
function History({incomes,expenses,categories,month,totalIn,totalOut,balance,goal}){
  ...
  const generatePDF=()=>{
    ...
    const incRows=incomes.map(i=>{
      ...
      return `<tr><td>${i.date}</td><td>${i.description}</td><td>${i.userName}</td><td>${cat.icon} ${cat.label}</td><td style="color:#10b981;text-align:right">+${fmt(i.amount)}</td></tr>`;
    }).join("");
    const expRows=expenses.map(e=>{
      const cat=categories.find(c=>c.id===e.category)||{label:e.category,icon:"📦"};
      return `<tr><td>${e.date}</td><td>${e.description}</td><td>${e.userName}</td><td>${cat.icon} ${cat.label}</td><td style="color:#ef4444;text-align:right">-${fmt(e.amount)}</td></tr>`;
    }).join("");
...
<style>
  *{box-sizing:border-box;}
  ...
  h2{color:#6366f1;font-size:18px;margin:28px 0 10px;border-bottom:2px solid #e2e8f0;padding-bottom:6px;}
  ...
  .card.green{background:#dcfce7;border:1px solid #86efac;}
  .card.red{background:#fee2e2;border:1px solid #fca5a5;}
  .card.blue{background:#ede9fe;border:1px solid #a5b4fc;}
  .card .val{font-size:20px;font-weight:900;margin:4px 0;}
  ...
  .meta{background:#ede9fe;border-radius:8px;padding:16px;margin:16px 0;}
...
<h1>💚 financeirU</h1>
<p style="color:#64748b;font-size:14px;">Relatório mensal — <strong>${monthLabel}</strong></p>
<div class="cards">
  <div class="card green"><div class="lbl">Entradas</div><div class="val" style="color:#16a34a">${fmt(totalIn)}</div><div class="lbl">${incomes.length} lançamentos</div></div>
  <div class="card red"><div class="lbl">Gastos</div><div class="val" style="color:#dc2626">${fmt(totalOut)}</div><div class="lbl">${expenses.length} lançamentos</div></div>
  <div class="card blue"><div class="lbl">Saldo</div><div class="val" style="color:${saved>=0?"#7c3aed":"#d97706"}">${fmt(saved)}</div><div class="lbl">${saved>=0?"✅ Positivo":"⚠️ Negativo"}</div></div>
</div>
<div class="meta">🎯 Meta de economia: <strong>${fmt(goal)}</strong> &nbsp;|&nbsp; Economizado: <strong>${fmt(saved)}</strong> &nbsp;|&nbsp; Progresso: <strong>${goalPct}</strong></div>
<h2>💰 Entradas</h2>
<table><thead><tr><th>Data</th><th>Descrição</th><th>Pessoa</th><th>Categoria</th><th style="text-align:right">Valor</th></tr></thead><tbody>${incRows||"<tr><td colspan=5 style='text-align:center;color:#94a3b8'>Nenhuma entrada</td></tr>"}</tbody></table>
<h2>💸 Gastos</h2>
<table><thead><tr><th>Data</th><th>Descrição</th><th>Pessoa</th><th>Categoria</th><th style="text-align:right">Valor</th></tr></thead><tbody>${expRows||"<tr><td colspan=5 style='text-align:center;color:#94a3b8'>Nenhum gasto</td></tr>"}</tbody></table>
<h2>🏷️ Gastos por categoria</h2>
<table><thead><tr><th>Categoria</th><th style="text-align:right">Valor</th><th style="text-align:right">% do total</th></tr></thead><tbody>${rows||"<tr><td colspan=3 style='text-align:center;color:#94a3b8'>Sem dados</td></tr>"}</tbody></table>
<div class="footer">Gerado pelo financeirU em ${new Date().toLocaleDateString("pt-BR")} • financeiro.app</div>
  ...
  return(
    <div>
      <div style={S.secHeader}>
        ...
        <button style={{...S.btnGreen,width:"auto",padding:"10px 16px",fontSize:13}} onClick={generatePDF}>
          📄 Gerar PDF
        </button>
      </div>
      ...
      {form && (
        <div style={S.card}>
          ...
          <div style={{display:"flex",gap:8,marginBottom:10}}>
            <input style={{...S.input,flex:"0 0 60px",textAlign:"center",fontSize:22,padding:"6px"}} value={icon} onChange={e=>setIcon(e.target.value)} placeholder="🏷️"/>
...
const S={
  ...
  loginTitle:{textAlign:"center",fontSize:26,fontWeight:900,letterSpacing:"-1px",color:"#10b981",margin:"0 0 4px"},
  loginSub:{textAlign:"center",color:"#64748b",fontSize:14,marginBottom:24},
  divider:{textAlign:"center",color:"#334155",margin:"14px 0",borderTop:"1px solid #334155",lineHeight:0},
  fg:{marginBottom:12},
  ...
  overlay:{position:"fixed",inset:0,background:"rgba(0,0,0,.7)",zIndex:100,display:"flex",alignItems:"flex-end",justifyContent:"center",padding:0},
